...
pub async fn api_login_user(credentials: &str) -> Result<UserLoginResponse, String> {
    let response = match http::Request::post("http://localhost:8001/api/auth/login")
        .header("Content-Type", "application/json")
...
pub async fn api_user_info() -> Result<User, String> {
    let response = match http::Request::get("http://localhost:8001/api/users/me")
        .credentials(http::RequestCredentials::Include)
        .send()
        .await
    {
...
pub async fn api_logout_user() -> Result<(), String> {
    let response = match http::Request::get("http://localhost:8001/api/auth/logout")
        .credentials(http::RequestCredentials::Include)
        .send()
        .await
    {
